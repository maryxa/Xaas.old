...
void XaUser::Dispatcher (string CalledEvent) {
    ...
    } else if (CalledEvent=="XaUserOrgTree"){
        this->XaUserOrgTree();
    } else if (CalledEvent=="XaUserOrgTreeView"){
        this->XaUserOrgTreeView();
...
void XaUser::XaUserLogin (){
	...
	if (StrEmail !="" && StrPassword !="") {
		...
		if (n==0){
			...
			LOG.Write("INF", __FILE__, __FUNCTION__,__LINE__,"User Is Valid -> Id is ->"+DbRes[0]["id"]+" and Session_ID is -> " +REQUEST.XaSession_ID);

			//UPDATE SESSION
			XaLibSql LibSql;

				vector<string> VectorFields{"XaUser_ID"};
				vector<string> VectorValues{DbRes[0]["id"]};
			...
			REQUEST.XaUser_ID=XaLibBase::FromStringToInt(DbRes[0]["id"]);
...
void XaUser::XaUserOrgTree (){
	...
	string XslParams[MAXITEMS] = {"TplType",TplType};
